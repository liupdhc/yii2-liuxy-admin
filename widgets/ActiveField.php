...
class ActiveField extends Component {
    ...
    public function __construct($config = []) {
        if (isset($config['inputOptions'])) {
            $config['inputOptions'] = ArrayHelper::merge($config['inputOptions'],['class'=>'form-control']);
        }
        if (isset($config['labelOptions'])) {
            $config['labelOptions'] = ArrayHelper::merge($config['labelOptions'],['class'=>'control-label']);
        }
        if (isset($config['errorOptions'])) {
            $config['errorOptions'] = ArrayHelper::merge($config['errorOptions'],['class'=>'help-block']);
        }
        if (isset($config['hintOptions'])) {
            $config['hintOptions'] = ArrayHelper::merge($config['hintOptions'],['class'=>'hint-block']);
        }
     ...
     *  'placeholder'=>'xxx'
     *  'class'=>'input-circle',//追加的class
     *  'icon'=>'fa fa-microphone',//支持输入框显示图标，默认为左边，如果要显示左边，增加icon-align=>'right'
     *  'icon-align'=>'right'//控制图标显示位置，须跟icon同时设置才有效，默认不设置为左边，需要注意input-circle、input-circle-right、input-circle三者的灵活组合
     * ]
     *
     * @return static the field object itself
     */
    public function textInput($options = []) {
        if (!$this->isHidden) {
            ...
            foreach($keys as $key) {
                if (in_array($key,self::$appendOptions) && isset($this->inputOptions[$key])) {
                    $this->inputOptions[$key] .= ' '.$options[$key];
                    unset($options[$key]);
                }
            }
        }
        ...
        if (isset($options['icon'])) {
            $result = $this->textInputIcon($options);
            $this->parts['{input}'] = Html::tag('div',$result[0],['class'=>$result[1]]);
        } else {
    ...
    public function passwordInput($options = []) {
        ...
        $this->adjustLabelFor($options);
        if (isset($options['icon'])) {
            $result = $this->textInputIcon($options, true);
            $this->parts['{input}'] = Html::tag('div',$result[0],['class'=>$result[1]]);
        } else {
